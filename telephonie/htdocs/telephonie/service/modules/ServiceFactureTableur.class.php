 ...
 *
 * $Id: ServiceFactureTableur.class.php,v 1.1 2009/10/20 16:19:25 eldy Exp $
 * $Source: /cvsroot/dolibarr/dolibarrmod/telephonie/htdocs/telephonie/service/modules/ServiceFactureTableur.class.php,v $
 *
 *
  ...
  Function Process($contrat)
    {
      ...
      /*
       * Recherche des factures comptables liées au contrat
       *
       */

      $sql2 = "SELECT distinct fk_facture";
      $sql2 .= " FROM ".MAIN_DB_PREFIX."telephonie_facture";
      $sql2 .= " WHERE fk_contrat = ".$contrat->id;
      $sql2 .= " AND date = '".$this->year."-".$this->month."-01'";

      $resql2 = $contrat->db->query($sql2);
      ...
      if ( $resql2 )
	{
	  ...
	  while ($j < $num2)
	    {
	      $row2 = $contrat->db->fetch_row($resql2);
	      
	      array_push($facids, $row2[0]);
	      $j++;
	    }
	}

      /*
       * Recherche des facture de téléphonie liée à la facture comptable
       *
       */
      ...
      foreach ($facids as $facid)
	{
	  ...
	  $facture = new Facture($contrat->db);
	  $facture->fetch($facid);
	  ...
	  $sql .= " WHERE fk_facture =".$facture->id;
	  ...
	  if ( $resql )
	    {
	      ...
	      while ($i < $num)
		{
		  $row = $contrat->db->fetch_row($resql);
		  array_push($factels, $row[0]);
		  $i++;
		}
	      $contrat->db->free($resql);
	    }
	  else
	    {
	      $error++;
	  ...
	  $fname = "/tmp/".$facture->ref.".xls";
	  
	  /*
	   * Génération du fchier tableur
	   */
	  
	  $workbook = &new writeexcel_workbook($fname);
	  
	  $page = &$workbook->addworksheet("Facture $facture->ref");
	  ...
	  foreach ($factels as $factel)
		...
		{
		  $i = 0;
		  $numsql = $contrat->db->num_rows($resql);
