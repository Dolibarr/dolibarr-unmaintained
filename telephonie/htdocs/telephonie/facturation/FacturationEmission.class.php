 ...
 *
 * $Id: FacturationEmission.class.php,v 1.9 2010/08/24 20:27:24 grandoc Exp $
...
class FacturationEmission {
  ...
  function Emission()
  {
    ...
    if (!$error)
      { 
	dol_syslog("FacturationEmission::Emission Nombre de contrats � facturer ".sizeof($contrats),LOG_DEBUG);
	array_push($this->messages, "Nombre de contrats � facturer : ".sizeof($contrats));
	$xcli = 0;
	...
	foreach ($contrats as $contrat)
	  {
	    ...
	    dol_syslog("FacturationEmission::Emission ".$xcli."/".$xclis." Contrat � facturer id=".$contrat." (".memory_get_usage() .")",LOG_DEBUG);
	    array_push($this->messages, $xcli."/".$xclis." Contrat � facturer id=".$contrat.",batch=".$batch_id);
		...
		dol_syslog("FacturationEmission::Emission Contrat $contrat, $i factures trouv�es � g�n�rer", LOG_DEBUG);
		array_push($this->messages, "Contrat $contrat, $i factures trouv�es � g�n�rer");
  ...
  function facture_contrat($db, $user, $contrat_id, $factel_ids, $datetime, &$factures_prev, &$factures_a_mailer)
  {
    ...
    if (!$error)
      {
	if ($verbose) dol_syslog("FacturationEmission::facture_contrat Cr�ation facture pour $soc->nom", LOG_DEBUG);
	...
	if ($facid > 0)
	  ...
	  {
	    dol_syslog("FacturationEmission::facture_contrat Erreur cr�ation objet facture erreur : $facid",LOG_ERR);
	    array_push($this->messages, array('error',"Erreur cr�ation objet facture erreur : $facid"));
	    $error = 16;
	  }		  
      }
	...
	foreach ($factel_ids as $factel_id)
	  {
	    ...
	    if ($factel->fetch($factel_id) == 0)
	      ...
	      {
		dol_syslog("FacturationEmission::facture_contrat Erreur lecture facture t�l�phonique $factel_id");
		array_push($this->messages, array('error',"Erreur lecture facture t�l�phonique (id=$factel_id)"));
		$error = 17;
	      }
	...
	foreach ($factel_ids as $factel_id)
	  {
		...
		if ($facdet->write_pdf_file($factel, $factel->ligne) == 0)
		  ...
		  {
		    dol_syslog("FacturationEmission::facture_contrat ERREUR lors de Generation du pdf detaille ($modele)");
		    $error = 19;
		  } 
	      }
