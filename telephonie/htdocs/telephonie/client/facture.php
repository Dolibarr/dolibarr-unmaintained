 ...
 *
 * $Id: facture.php,v 1.1 2009/10/20 16:19:27 eldy Exp $
...
if ($_POST["action"] == 'send' || $_POST["action"] == 'relance')
{
    ...
    if ( $fac->fetch($_POST["facid"]) )
    {
      ...
      {
        dol_syslog("Impossible de lire les donn�es de la facture. Le fichier facture n'a peut-�tre pas �t� g�n�r�.");
      }
}
  ...
  if ( $fac->fetch($_GET["facid"], $user->societe_id) > 0)
      ...
      if ($user->societe_id == 0 && $_GET["action"] <> 'valid' && $_GET["action"] <> 'editline')
	{
	  ...
	  */
      ...
      /*
       * Documents g�n�r�s
       * Le fichier de facture d�taill�e est de la forme
