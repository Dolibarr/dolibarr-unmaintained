 ...
 *
 *
 * Génére un tableur des lignes commandées dans la semaine
 * On génère sur la semaine précédente, le script étant lancé le lundi
 *
 */
...
$dateg = $date - (86400 * 7) ; // On génére la semaine précédente

Generate($dateg, $db, $date_now, $email);
...
{
  $datef = strftime("%Y-%V", $date_gen);

  $shortname = "recap-semaine-".$datef.".xls";
  ...
  $worksheet->write(0, 0,  "Récapitulatif des commandes semaine ".strftime("%V/%Y",$date_gen)." (généré le ".strftime("%d %B %Y %HH%M", $date_now) . ")");
