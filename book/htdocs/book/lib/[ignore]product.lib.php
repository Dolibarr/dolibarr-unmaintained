...
/**
 ...
 \version    $Id: [ignore]product.lib.php,v 1.1 2010/05/31 15:28:24 pit Exp $
 */
...
function product_prepare_head($product, $user)
{
  ...
  //affichage onglet book si le module book est activé et si le produit est un book
  if ($conf->book->enabled)
  {
	  $head[$h][0] = DOL_URL_ROOT."/product/book/show_book.php?id=".$product->id;
	  $head[$h][1] = $langs->trans("Book");
	  $head[$h][2] = 'book';
	  $h++;  
  }
